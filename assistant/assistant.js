var btnsubmit, txtname, txtprompt, txtsearchfields, chkstream, chkagent, chkretrieval, richresultstoggle, sectionlinks, sectionassistant, sectionassistantstatus, filesToUpload, divgoogle, links, assistantlist, webtogglebutton, systemassistantlist, sectionsystemassistant, txtuserprompt;
let user;
...
async function saveassistant() {
    ...
    if (txtname.value.length > 0 && txtprompt.value.length > 0) {
        ...
        if (filesToUpload.files.length > 0)
            filesurls = await uplaodfiles(filesToUpload.files);
...
function getassistant() {
            ...
            prompts.forEach(function (item) {
                ...
                if (item.Status == "Created")
                    if ((item.Files && item.Files.length > 0) || (item.URL && item.URL.length > 0))
                        str = str + '<button class="button" onclick="startindexing(\'' + item.Id + '\')"><img src="./images/index.svg"></button>';
                    else
                        str = str + '<button class="button" onclick="deploy(\'' + item.Id + '\')"><img src="./images/activate.svg"></button>';
                else if (item.Status == "Indexing")
                    str = str + '<button class="button" onclick="updateindexstatus(\'' + item.Id + '\',\'' + item.AgentSessionID + '\')"><img src="./images/indexing.svg"></button>';
                ...
                str = str + '&nbsp;&nbsp;&nbsp;<button class="button" onclick="toggleCollapse(this)"><img src="./images/down.svg"></button>';
                str = str + '</div></header><section>';
                str = str + '<p><span class="ptitle">Prompt:</span><br><span class="pbody">' + truncate(item.Prompt) + '</span></p>';
                str = str + '<p><span class="ptitle">Retrieval:</span><br><span class="pbody">' + item.Retrieval + '</span></p>';
                str = str + '<p><span class="ptitle">Web Search:</span><br><span class="pbody">' + item.WebSearch + '</span></p>';
                str = str + '<p><span class="ptitle">Rich Results:</span><br><span class="pbody">' + item.rich_results + '</span></p>';
                str = str + '<p><span class="ptitle">ChatbotURL:</span><br><span class="pbody"><a href="' + item.ChatbotURL + '" target="_blank" class="url-list">' + item.ChatbotURL + '</a></span></p>';
                ...
                str = str + '</span></p><p><span class="ptitle">Search Fields:</span><br><span class="pbody">' + item.store_search_fields + '</span></p>';
                str = str + '</span></p></section><footer>';
                if (item.Status == "Deployed")
                    str = str + '<button class="button"  onclick="deactivate(\'' + item.Id + '\')"><img src="./images/deactivate.svg"></button>';

                str = str + '<button class="button" onclick="deleteassistant(\'' + item.Id + '\')"><img src="./images/delete.svg"></button>';
                str = str + '</footer></div>';
    ...
    xhr.open("GET", "https://ai.iamplus.services/api/v1/db/data/v1/UserAssistant/Assistant?limit=25&shuffle=0&offset=0&where=(UUID%2Ceq%2C" + user.uuid + ")");
    xhr.setRequestHeader("accept", "application/json");
    xhr.setRequestHeader("xc-token", "dylFE2UGqVNqHZR0OtruRDJh2UKNxVbitJwQvp1x");
...
function getsystemassistant() {
    ...
    xhr.addEventListener("readystatechange", function () {
        if (this.readyState === 4) {
            ...
            prompts.forEach(function (item) {
                ...
                str = str + '<header><h2>' + item.Title + '</h2>';
                str = str + '<div class="headerbuttons">';
                str = str + '<button class="button" onclick="duplicate(\'' + item.Id + '\')"><img src="./images/duplicate.svg"></button>';
                ...
                str = str + '</span></p>';
                str = str + '<p><span class="ptitle">Search Fields:</span><br><span class="pbody">' + item.store_search_fields + '</span></p>';
    ...
    xhr.open("GET", "https://ai.iamplus.services/api/v1/db/data/v1/UserAssistant/SystemAssistant?limit=250&shuffle=0&offset=0");
    xhr.setRequestHeader("accept", "application/json");
    xhr.setRequestHeader("xc-token", "dylFE2UGqVNqHZR0OtruRDJh2UKNxVbitJwQvp1x");
...
async function startindexing(itemid) {
    ...
    if (assistant.URL && assistant.URL.length > 0)
        if (assistant.URL.includes(";"))
            aurls = assistant.URL.split(";")
        else
            aurls[0] = assistant.URL

    if (assistant.Files && assistant.Files.length > 0)
        if (assistant.Files.includes(","))
            links = JSON.parse(assistant.Files).split(",");
        else
            links[0] = assistant.Files
    ...
    if (assistant.Retrieval == "true") {
        ...
        xhr.addEventListener("readystatechange", function () {
            if (this.readyState === 4) {
                console.log(this.responseText);
                let data = JSON.parse(this.responseText);
                updateassistant(itemid, "Deployed", URL + data.session_id + "&deploy_id=" + deploy_id, data.session_id, data.stream_id, assistant.data_store_name, deploy_id);
            }
        });
...
const getassistant_itemid = itemid => new Promise(function (resolve, reject) {
    // WARNING: For GET requests, body is set to null by browsers.
...
const getsystemassistant_itemid = itemid => new Promise(function (resolve, reject) {
    // WARNING: For GET requests, body is set to null by browsers.

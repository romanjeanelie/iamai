...
class Chat {
  ...
  async submitmovieselection(MovieTitle, Theatre, Date2, Time) {
    ...
    for (let flightsi = 0; flightsi < 2; flightsi++) {
      ...
      if (FlightSearchResultsArr.length > 0) {
        ...
        FlightSearchResultsArr.forEach((FlightSearchResult) => {
          ...
          timeHourMinP.innerHTML = FlightSearchResult.travel.duration.split("h")[0] + '<span style="color: rgba(0, 0, 0, 0.40);">h</span> ' + FlightSearchResult.travel.duration.substring(

...
class PhoneAnimations {
  ...
  toConnected() {
    ...
    this.dotsYoyoAnimations.forEach((anim2) => anim2.cancel());
  ...
  toListening() {
    var _a;
    (_a = this.phoneBarYoyoAnimations) == null ? void 0 : _a.cancel();
  ...
  toProcessing() {
    ...
    reducePhoneBarOne.onfinish = () => {
      ...
      this.dotsYoyoAnimations = Array.from(this.phoneBarProcessingDots).map(
        (dot) => this.createAnimation(dot, this.keyframes.dotsYoyo, {
          delay: 500,
          duration: 1500,
          ease: "ease-out",
          iterations: Infinity
  ...
  resetAIBar() {
    var _a;
    this.phoneBarAI.classList.remove("active");
    (_a = this.phoneBarAIYoyoAnimations) == null ? void 0 : _a.cancel();
